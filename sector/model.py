...
class WeightedMeanConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_weight):
        ...
        deg_w = scatter_add(edge_weight, dst, dim=0, dim_size=x.size(0)).clamp_min(1e-12)
...
class SEModel(nn.Module):
    ...
    def calculate_se_loss(self):
        ...
        deg   = scatter_sum(w, i, dim=0, dim_size=N)           # (N,)
        vol_G = deg.sum()
        ...
        mask_self = (i == j)
        w_self = scatter_sum(w[mask_self], i[mask_self], dim=0, dim_size=N)
    ...
    def calculate_balance_loss(self):
        ...
        if self.balance_mode == "volume":
            g = self.g_dic[self.height]
            deg = scatter_sum(g.edge_weight, g.edge_index[0], dim_size=self.num_nodes)
